...
class UsersController extends AdminController
{
    ...
    public function home(?array $data): void
    {
        /**
         * Search redirect
         * pt-br: Busca de usuários por palavra chave
        */
        if(!empty($data["s"])){
            $s = str_search($data["s"]);
        ...
        $search = null;
        $id = Auth::user()->id;
        $users = (new User())->find("id != :id", "id={$id}");
    ...
    public function user(?array $data): void
    {
        ...
        */
        ...
        if(!empty($data["action"]) && $data["action"] == "update"){
            ...
            if(!$userUpdate){
               $this->message->warning("Você tentou gerenciar um usuário que não existe ou foi deletado.")->flash();
            ...
            if(!empty($_FILES["photo"])){
                ...
                if($userUpdate->photo && file_exists(__DIR__ . "/../../../storage/".CONF_UPLOAD_DIR."/{$userUpdate->photo}")){
                    unlink(__DIR__ . "/../../../storage/".CONF_UPLOAD_DIR."/{$userUpdate->photo}");
        ...
        if(!empty($data["action"]) && $data["action"] == "delete"){
            ...
            if(!$userDelete){
                $this->message->warning("Você tentou deletar um usuário que não existe.")->flash();
                echo json_encode(["redirect"=>url("/admin/users/home")]);
                return;
            }

            if($userDelete->photo && file_exists(__DIR__ . "/../../../storage/".CONF_UPLOAD_DIR."/{$userDelete->photo}")){
                unlink(__DIR__ . "/../../../storage/".CONF_UPLOAD_DIR."/{$userDelete->photo}");
